...
-- ===========
-- Listar todos os alunos com seus nomes
-- ===========
SELECT a.id_aluno, a.ra, p.nome_completo
FROM Aluno a
JOIN Pessoa p ON a.id_pessoa = p.id_pessoa;

-- ===========
-- Listar professores e as disciplinas que eles lecionam
-- ===========
SELECT p.nome_completo, pr.area_atuacao, d.nome AS disciplina
FROM Professor pr
JOIN Pessoa p ON pr.id_pessoa = p.id_pessoa
JOIN Professor_Disciplina pd ON pr.id_professor = pd.id_professor
JOIN Disciplina d ON pd.id_disciplina = d.id_disciplina;

-- ===========
-- Listar alunos e as turmas em que estão matriculados
-- ===========
SELECT p.nome_completo, a.ra, t.sala, t.ano, t.semestre
FROM Matricula m
...
-- ===========
-- Mostrar histórico escolar de cada aluno
-- ===========
SELECT p.nome_completo, d.nome AS disciplina, h.nota, h.frequencia, h.estado
FROM Historico_Escolar h
JOIN Matricula m ON h.id_matricula = m.id_matricula
JOIN Aluno a ON m.id_aluno = a.id_aluno
JOIN Pessoa p ON a.id_pessoa = p.id_pessoa
JOIN Disciplina d ON h.id_disciplina = d.id_disciplina;

-- ===========
-- Cursos e os professores responsáveis
-- ===========
SELECT c.nome AS curso, p.nome_completo AS professor
FROM Curso c
...
-- ===========
-- Corrigir situação da matrícula
-- ===========
UPDATE Matricula
SET situacao = 'Ativo'
...
-- ===========
-- Atualizar nota de uma disciplina no histórico
-- ===========
UPDATE Historico_Escolar
SET nota = 7.5, estado = 'Aprovado'
